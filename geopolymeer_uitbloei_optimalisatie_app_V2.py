...
# Uitbloei risico inschatting (simpele interpretatie)
aantal_afwijkingen = sum(df["Afwijking (%)"] > 10)
if aantal_afwijkingen == 0:
    risico_uitbloei = "Laag (<10%)"
elif aantal_afwijkingen == 1:
    risico_uitbloei = "Matig (10-20%)"
else:
    risico_uitbloei = "Hoog (>20%)"

st.subheader("4. Risico op uitbloei")
st.success(f"Risico-inschatting: {risico_uitbloei}")

# Persbaarheidsscore (eenvoudige inschatting o.b.v. H2O/SiO2 verhouding)
if h2o_sio2 < 0.18:
...
# Keuze voor nieuwe input
st.subheader("6. Nieuwe invoer?")
nieuwe_input = st.radio("Wil je een nieuwe menging ingeven?", ["Nee", "Ja"])
if nieuwe_input == "Ja":
