...
st.title("Hybrid Geopolymer Optimization (GGBFS fixed)")

st.markdown("This tool supports hybrid systems using Metakaolin and GGBFS. Molar ratios are compared with literature values for strength, efflorescence risk, and workability. Optimization is based on fixed GGBFS input. Negative values are prevented; efflorescence risk takes priority over pressability.")
...
st.header("3. Optimization (GGBFS fixed)")
if st.button("Optimize"):
    mol_Ca_opt = mol_Ca
    mol_Si_opt = mol_Ca_opt / ideal["Ca/Si"]
    mol_Al_opt = mol_Si_opt / ideal["Si/Al"]
    ...
    m_MK_opt = (mol_Al_opt * MW["Al2O3"]) / (2 * metapor["Al2O3"])
    m_H2O_opt = mol_H2O_opt * MW["H2O"]
    m_H2O_extra_opt = m_H2O_opt - (m_NaOH_opt * 0.5 + m_sil_opt * 0.665)

    if any(x < 0 for x in [m_MK_opt, m_sil_opt, m_H2O_extra_opt]):
        ...
        st.write(pd.DataFrame({
            ...
            "Mass": [round(m_MK_opt, 1), round(m_ggbs, 1), round(m_NaOH_opt, 1), round(m_sil_opt, 1), round(m_H2O_extra_opt, 1)]
        }))
